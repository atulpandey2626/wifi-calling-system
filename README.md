...
## How it works
1. Every device broadcasts a UDP signal every 2 seconds so other devices on the same WiFi can find it2. Once a device is selected, a TCP connection is used to exchange WebRTC connection details (SDP/ICE candidates)3. WebRTC then handles the actual audio call - echo cancellation, noise suppression, etc happen automatically
...
## Running it
1. `flutter pub get`2. Connect two Android phones to the same WiFi3. Allow microphone and network permissions4. `flutter run --release`
